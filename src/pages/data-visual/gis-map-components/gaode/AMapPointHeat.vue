  ...
  export default {
    ...
    methods: {
      ...
      addHeatOverlay () {
        let vue = this
        if (!vue.heatOverlay) {
          vue.heatOverlay = new AMap.Heatmap(vue.bmap, vue.heatStyle)
        }
        let points = []
        vue.data.forEach(p => {
          if (p.gps) {
      ...
      changePointHeatSlider (value) { // 散点热力图权重滑动条
        if (this.heatOverlay) {
          this.heatOverlay.setDataSet({data: this.heatOverlay.nh.data, max: (value === 0 ? 1 : value)})
        }
      },
      removeOverlays () {
        if (this.heatOverlay) {
